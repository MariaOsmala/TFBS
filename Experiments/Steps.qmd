...
Note that this will extract the old info for motif representativenes from the supplementary
data of the previous publications. The updated info for representativeness can be added once the minimum dominating set analysis has been performed. The code `code/SELEX-Dominating-Set-Analysis/process_DSA_results.R` generates `Data/SELEX-motif-collection/metadata_representative.tsv`
```{r}
#| eval: false
#| include: false
