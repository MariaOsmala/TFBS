...
# cCRE_hg38.tsv.gz What is this? Not used in Xie et al. 2025
...
#Cell_ontology=read.table(paste0(data_path,"Cell_ontology.tsv"),sep="\t", header=TRUE) Not used

#Cell_metadata.tsv.gz where is this, not used in Xie et al. 2025
# ====================
#   
...
# Celltypes=read.table(paste0(data_path, "celltypes.txt.gz"),sep="\t", header=FALSE) not used?

# * 435,142 adult cCREs with restricted accessibility in one or a few cell types


#From Mendeley database
files=dir(paste0(data_path, "Zhang et al Figure 2/2E_Cell_type_restricted_peaks/"))
...
for(ct in ct_names){
  print(ct)
  cCREs_list[[ct]]=import(paste0(data_path,"Zhang et al Figure 2/2E_Cell_type_restricted_peaks/",ct,".bed.gz"),format="bed")
...
#Experimental or TODO stuff below
...
summary=read.csv(paste0(data_path, "/Zhang et al Figure 4/Diff_peaks_motifs/summary.tsv"), sep="\t", header=TRUE)
