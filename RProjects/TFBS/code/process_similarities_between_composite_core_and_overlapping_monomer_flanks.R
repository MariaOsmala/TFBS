...
#save.image(file="/scratch/project_2006203/TFBS/RProjects/TFBS/RData/kmer_score_computations_another_way.RData")
#load(file="/scratch/project_2006203/TFBS/RProjects/TFBS/RData/kmer_score_computations_another_way.RData")
...
composites_correct_table_all %>% filter(sj_divergence_flank_TF1_core>0.24 & sj_divergence_flank_TF1_core<0.25 &
                                          sj_divergence_flank_TF2_core>0.24 & sj_divergence_flank_TF2_core<0.25) %>% pull(composite)

composites_correct_table_all %>% filter(sj_divergence_flank_TF1_core>0.45 & sj_divergence_flank_TF1_core<0.55 &
                                          sj_divergence_flank_TF2_core>0.45 & sj_divergence_flank_TF2_core<0.55) %>% pull(composite)

composites_correct_table_all %>% filter(sj_divergence_flank_TF1_core>0.73 & sj_divergence_flank_TF1_core<0.77 &
                                          sj_divergence_flank_TF2_core>0.73 & sj_divergence_flank_TF2_core<0.77) %>% pull(composite)



composites_correct_table_all %>% filter(sj_divergence_flank_TF1_core>0.95 & sj_divergence_flank_TF1_core<1.05 &
                                          sj_divergence_flank_TF2_core>0.95 & sj_divergence_flank_TF2_core<1.05) %>% pull(composite)

composites_correct_table_all %>% filter(sj_divergence_flank_TF1_core>1.45 & sj_divergence_flank_TF1_core<1.55 &
                                          sj_divergence_flank_TF2_core>1.45 & sj_divergence_flank_TF2_core<1.55) %>% pull(composite)

composites_correct_table_all %>% filter(sj_divergence_flank_TF1_core>1.95 & sj_divergence_flank_TF1_core<2.05 &
                                          sj_divergence_flank_TF2_core>2.45 & sj_divergence_flank_TF2_core<2.55) %>% pull(composite)


#Draw a histogram from pre-computed values 

ggplot(composites_correct_table_all, aes(x=kl_divergence_mean)) + geom_histogram(binwidth=0.05)
...
for( i in 1:100){
  gm_result_2[[i]] <- Mclust(composites_correct_table_all$kl_divergence_mean, G=2, modelNames="V")
}
...
library("rootSolve")
df.pp<-data.frame(mean_kl_divergence = composites_correct_table_all$kl_divergence_mean)
...
table(composites_correct_table_all$kl_divergence_mean>mean_kl_divergence_threshold) #10 %
#FALSE  TRUE 
#921   102 
table(composites_correct_table_all$kl_divergence_mean>1) #38 %
#FALSE  TRUE 
#630   393 

table(composites_correct_table_all$kl_divergence_mean>2) #7.6 %
