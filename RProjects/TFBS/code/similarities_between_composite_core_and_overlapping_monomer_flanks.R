...
composites_correct_table=data.frame(composite=character(),
                                    first.monomer=character(), 
                                    second.monomer=character(), 
                                    TF1_start=numeric(), 
                                    TF1_end=numeric(), 
                                    TF1_orientation=character(),
                                    TF1_length=numeric(),
                                    TF1_start_monomer=character(),
                                    TF1_end_monomer=character(),
                                    TF2_start=numeric(), 
                                    TF2_end=numeric(),
                                    TF2_orientation=character(),
                                    TF2_length=numeric(),
                                    TF2_start_monomer=character(),
                                    TF2_end_monomer=character(),
                                    overlap=numeric(),
                                    overlap_start=numeric(), 
                                    overlap_end=numeric(), 
                                    overlap_not_between_flanks=logical(),
                                    kl_divergence_core_flank_TF1=numeric(),
                                    kl_divergence_flank_TF1_core=numeric(),
                                    kl_divergence_mean_TF1=numeric(),
                                    kl_divergence_core_flank_TF2=numeric(),
                                    kl_divergence_flank_TF2_core=numeric(),
                                    kl_divergence_mean_TF2=numeric(), 
                                    sj_divergence_flank_TF1_core=numeric(),
                                    sj_divergence_flank_TF2_core=numeric(),
...
for(i in 869:nrow(composites)){
  ...
  composites_correct_table=rbind(composites_correct_table, 
                                 data.frame(composite=composites$ID[i],
...
save.image(file="~/projects/TFBS/RProjects/TFBS/RData/kmer_score_computations.RData")
...
#composites_correct_table$kl_divergence_mean_TF1= composites_correct_table$kl_divergence_mean_TF1/composites_correct_table$overlap
#composites_correct_table$kl_divergence_mean_TF2= composites_correct_table$kl_divergence_mean_TF2/composites_correct_table$overlap

#composites_correct_table$kl_divergence_mean=rowMeans( composites_correct_table[,c("kl_divergence_mean_TF1","kl_divergence_mean_TF2")] )

#sort accordint to the mean kl-divergence

#composites_correct_table=composites_correct_table[order(composites_correct_table$kl_divergence_mean),]

saveRDS(composites_correct_table, file="~/projects/TFBS/RProjects/TFBS/RData/composites_correct_table.RDS")
#composites_correct_table=readRDS(file="~/projects/TFBS/RProjects/TFBS/RData/composites_correct_table.RDS")

#Filter out those cases where the overlap does not correspond to flanks

composites_correct_table=composites_correct_table %>% filter(overlap_not_between_flanks==FALSE)  #977

#Take the minimum of the kl-divergence between the core and the flanks


composites_correct_table$kl_divergence_min_TF1=apply(composites_correct_table[,c("kl_divergence_core_flank_TF1", "kl_divergence_flank_TF1_core")],1,min)
composites_correct_table$kl_divergence_min_TF2=apply(composites_correct_table[,c("kl_divergence_core_flank_TF2", "kl_divergence_flank_TF2_core")],1,min)

composites_correct_table$kl_divergence_min_TF1=composites_correct_table$kl_divergence_min_TF1/composites_correct_table$overlap
composites_correct_table$kl_divergence_min_TF2=composites_correct_table$kl_divergence_min_TF2/composites_correct_table$overlap


#Divide the sj_divergence by overlap length
composites_correct_table$sj_divergence_flank_TF1_core=composites_correct_table$sj_divergence_flank_TF1_core/composites_correct_table$overlap
composites_correct_table$sj_divergence_flank_TF2_core=composites_correct_table$sj_divergence_flank_TF2_core/composites_correct_table$overlap

#Draw a scatter plot KL-divergences

ggplot(composites_correct_table, 
       aes(x=kl_divergence_min_TF1, y=kl_divergence_min_TF2)) + geom_point()+
  theme_minimal()

#Draw a scatter plot JS-divergences
ggplot(composites_correct_table, 
       aes(x=sj_divergence_flank_TF1_core, y=sj_divergence_flank_TF2_core)) + geom_point()+
  theme_minimal()

which(composites_correct_table$sj_divergence_flank_TF1_core>3 & composites_correct_table$sj_divergence_flank_TF2_core>3)
which(composites_correct_table$sj_divergence_flank_TF1_core>4 & composites_correct_table$sj_divergence_flank_TF2_core>1)
which(composites_correct_table$sj_divergence_flank_TF1_core<0.3 & composites_correct_table$sj_divergence_flank_TF2_core>4)
which(composites_correct_table$sj_divergence_flank_TF1_core<1 & composites_correct_table$sj_divergence_flank_TF2_core<1)


#Draw a histogram from pre-computed values 

ggplot(composites_correct_table, aes(x=kl_divergence_mean)) + geom_histogram(binwidth=0.05)
...
for( i in 1:100){
  gm_result_2[[i]] <- Mclust(composites_correct_table$kl_divergence_mean, G=2, modelNames="V")
}

#Choose the one with the best (maximum) BIC

best_fit=gm_result_2[[which.max(sapply( gm_result_2, function(x) x$bic))]]


# Define parameters
mu1 <- best_fit$parameters$mean[1]
sigma1 <- sqrt(best_fit$parameters$variance$sigmasq[1])
mu2 <- best_fit$parameters$mean[2]
sigma2 <- sqrt(best_fit$parameters$variance$sigmasq[2])
mu3 <- best_fit$parameters$mean[3]
sigma3 <- sqrt(best_fit$parameters$variance$sigmasq[3])


lambda1=best_fit$parameters$pro[1]
lambda2=best_fit$parameters$pro[2]
lambda3=best_fit$parameters$pro[3]


install.packages("rootSolve")
library("rootSolve")
df.pp<-data.frame(mean_kl_divergence = composites_correct_table$kl_divergence_mean)

find_intersection <- function(mu1, sigma1, pi1, mu2, sigma2, pi2) {
  # Function representing the difference between the two Gaussian distributions
  diff_gaussians <- function(x) {
    N1 <- pi1 *  dnorm(x, mean=mu1, sd=sigma1) #dnorm(x, mean = mean, sd = sd)) * n * binwidth * lambda
    N2 <- pi2 *  dnorm(x, mean=mu2, sd=sigma2)
    return(N1 - N2)
  }
  
  # Use `uniroot.all` from `rootSolve` package to find all roots within the range
  roots <- uniroot.all(diff_gaussians, c(mu1 - 5*sigma1, mu2 + 5*sigma2))
  return(roots)
}

intersections <- find_intersection(mu1, sigma1, lambda1, mu2, sigma2, lambda2)



ggplot(df.pp, aes(x =mean_kl_divergence )) +
  ...
  mapply(
    ...
    mean = best_fit$parameters$mean, #mean
    sd = sqrt(best_fit$parameters$variance$sigmasq), #standard deviation
    n = length(df.pp$mean_kl_divergence), #sample size
    lambda=best_fit$parameters$pro,
    binwidth = 0.02 #binwidth used for histogram
  ) + theme_minimal()+ labs(y="frequency", title="XXX")+geom_vline(aes(xintercept=max(intersections)), color="green", linetype="dotdash")

mean_kl_divergence_threshold=max(intersections)


#How many composite motifs have a mean kl-divergence above the threshold?

table(composites_correct_table$kl_divergence_mean>mean_kl_divergence_threshold) #10 %
#FALSE  TRUE 
#921   102 
table(composites_correct_table$kl_divergence_mean>1) #38 %
#FALSE  TRUE 
#630   393 

table(composites_correct_table$kl_divergence_mean>2) #7.6 %
